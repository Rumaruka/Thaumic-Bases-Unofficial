...
public class RevolverUpgrade {
    ...
    public RevolverUpgrade(int id, String name)
    {
        ...
        {
            FMLLog.warning("[TB]Attempting to register revolver upgrade "+name+"["+id+"], but the ID is already occupied by "+allUpgrades[id].text_id+"["+allUpgrades[id].id+"], ignoring.");
            return;
        }
    ...
    {
        return I18n.translateToLocal("revolverUpgrade."+text_id+".name");
    }
    ...
    public float modifyDamage_start(EntityLivingBase base, ItemStack revolver, float currentModification, int modLevel)
    {
        if(base != null)
        {
            ...
            if(this == silver && (base.getCreatureAttribute() == EnumCreatureAttribute.UNDEAD || isBMEntity(base) || (base instanceof EntityPlayer && isWerewolf((EntityPlayer) base))))
