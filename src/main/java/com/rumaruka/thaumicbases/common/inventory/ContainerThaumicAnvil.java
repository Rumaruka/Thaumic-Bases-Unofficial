...
import com.rumaruka.thaumicbases.common.block.BlockThaumicAnvil;
...
public class ContainerThaumicAnvil extends ContainerRepair{
	...
	public ContainerThaumicAnvil(InventoryPlayer inv, final World w, final BlockPos pos, EntityPlayer p)
	{
        ...
        this.addSlotToContainer(new Slot(this.outputSlot, 2, 134, 47)
        {
            ...
            {
                return (playerIn.capabilities.isCreativeMode || playerIn.experienceLevel >= ContainerThaumicAnvil.this.maximumCost) && ContainerThaumicAnvil.this.maximumCost > 0 && this.getHasStack();
            }
            public ItemStack onTake(EntityPlayer thePlayer, ItemStack stack)
            {
                if (!thePlayer.capabilities.isCreativeMode)
                {
                    thePlayer.addExperienceLevel(-ContainerThaumicAnvil.this.maximumCost);
                }

                float breakChance = onAnvilRepair(thePlayer, stack, ContainerThaumicAnvil.this.inputSlots.getStackInSlot(0), ContainerThaumicAnvil.this.inputSlots.getStackInSlot(1));

                ContainerThaumicAnvil.this.inputSlots.setInventorySlotContents(0, ItemStack.EMPTY);

                if (ContainerThaumicAnvil.this.materialCost > 0)
                {
                    ItemStack itemstack = ContainerThaumicAnvil.this.inputSlots.getStackInSlot(1);

                    if (!itemstack.isEmpty() && itemstack.getCount() > ContainerThaumicAnvil.this.materialCost)
                    {
                        itemstack.shrink(ContainerThaumicAnvil.this.materialCost);
                        ContainerThaumicAnvil.this.inputSlots.setInventorySlotContents(1, itemstack);
                    }
                    else
                    {
                        ContainerThaumicAnvil.this.inputSlots.setInventorySlotContents(1, ItemStack.EMPTY);
                    }
                }
                else
                {
                ...
                if (!thePlayer.capabilities.isCreativeMode && !w.isRemote && iblockstate.getBlock() == TBBlocks.thaumicAnvil && thePlayer.getRNG().nextFloat() < breakChance)
                {
                    int l = iblockstate.getValue(BlockThaumicAnvil.DAMAGE);
                    ++l;
                    ...
                    {
                        w.setBlockState(pos, iblockstate.withProperty(BlockThaumicAnvil.DAMAGE, l), 2);
    ...
    {
        if (this.w.getBlockState(new BlockPos(x, y, z)).getBlock() != TBBlocks.thaumicAnvil)
        {
            return false;
        }
        else
